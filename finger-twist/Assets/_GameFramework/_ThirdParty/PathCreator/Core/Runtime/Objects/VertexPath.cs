...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public Vector3 GetTangent(int index) => MathUtility.TransformDirection(localTangents[index], transform, space);

        public Vector3 GetNormal(int index) => MathUtility.TransformDirection(localNormals[index], transform, space);

        public Vector3 GetPoint(int index) => MathUtility.TransformPoint(localPoints[index], transform, space);
        ...
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            var direction = Vector3.Lerp(localTangents[data.previousIndex], localTangents[data.nextIndex], data.percentBetweenIndices);
            return MathUtility.TransformDirection(direction, transform, space);
        }
        ...
        private TimeOnPathData CalculateClosestPointOnPathData(Vector3 worldPoint)
        {
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                var closestPointOnSegment = MathUtility.ClosestPointOnLineSegment(worldPoint, GetPoint(i), GetPoint(nextI));
